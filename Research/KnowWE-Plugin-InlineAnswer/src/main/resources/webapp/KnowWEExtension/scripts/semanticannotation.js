...
/**
 * Namespace: KNOWWE.plugin.semanticannotation
 * The KNOWWE plugin d3web namespace.
 * Initialized empty to ensure existence.
 */
KNOWWE.plugin.semanticannotation = function(){
    return {
    }
}();


KNOWWE.plugin.semanticannotation.rendering = function() {
	...
	return {
        ...
        init : function(){
            if(_KS('.semLink').length != 0){
                _KS('.semLink').each(function(element) {
                    _KE.add('click', element, KNOWWE.plugin.semanticannotation.rendering.showOverlayQuestion);
                });
            }
        },
        ...
        showOverlayQuestion : function(e){
            ...
            var olay = new KNOWWE.helper.overlay({
                ...
                fn : KNOWWE.plugin.semanticannotation.rendering.overlayActions
             });
        },
        ...
        overlayActions : function(){
            if(_KS('.semano_mc').length != 0){
                _KS('.semano_mc').each(function(element){
                    _KE.add('click', element, KNOWWE.plugin.semanticannotation.rendering.handleForm);
                });
            }

            if(_KS('.semano_oc').length != 0){
                _KS('.semano_oc').each(function(element){
                    _KE.add('click', element, KNOWWE.plugin.semanticannotation.rendering.handleOC);
                });         
            }
            if(_KS('.semano_num').length != 0){
                _KS('.semano_num').each(function(element){
                    _KE.add('keydown', element, KNOWWE.plugin.semanticannotation.rendering.handleNum);
                });
                 _KS('.semano_ok').each(function(element){
                    _KE.add('click', element, KNOWWE.plugin.semanticannotation.rendering.handleNum);
                });
            }
            if(_KS('#o-lay')){
        ...
        handleNum : function(e){            
            ...
            if( !rel ) return;            
            KNOWWE.plugin.semanticannotation.rendering.send( rel, {ValueNum : el.value} );
        },
        ...
        handleOC : function(e){
            ...
            KNOWWE.plugin.semanticannotation.rendering.send( rel, null );
        },
...
(function init(){ 
	...
	if( KNOWWE.helper.loadCheck( ['Wiki.jsp'] )){
        window.addEvent( 'domready', function(){           
            var ns = KNOWWE.plugin.semanticannotation.rendering.init();         
        });
     }
}());