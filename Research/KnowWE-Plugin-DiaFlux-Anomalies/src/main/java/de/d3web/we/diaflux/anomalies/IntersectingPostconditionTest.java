		...
		if(null != kb) {
			...
			for(Flow flow : flowcharts) {
				for(Node node : flow.getNodes()) {
					...
					if(node.getClass().equals(ActionNode.class)) { //richtige Klasse raussuchen
						List<Condition> conditions = new LinkedList<Condition>();
						for(Edge edge : node.getOutgoingEdges()) {
//							System.out.println(edge.getCondition().toString()); //richtige Conditions raussuchen
...
//									Method meth = cls.getm
									System.out.println("Class " + condition.getClass() + " string " + condition.toString());
								}
							}
						}	
					}
		...
		if(!errormsg.isEmpty()) {
			res = new CITestResult(Type.FAILED, "Postconditions are intersecting: " + errormsg, config);
		}
		return res;
	}
