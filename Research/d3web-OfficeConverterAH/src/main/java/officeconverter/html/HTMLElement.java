...
public class HTMLElement {
	...
	private Element getElementName(org.jdom.Element elem) {
		...
		if ("h".equals(name)) {
			...
			if (frame != null && "frame".equals(frame.getName())) {
				...
				String clipping = StyleManager.getInstance().getStyle(frame,
						Style.STYLE_TYPE_CLIPPING);
				ret.setAttribute(
						ret.getAttribute() + " clip=\"" + clipping + "\"");

				String y = "0.00cm", width = "0.00cm", height = "0.00cm", zindex = "0.00cm";
				for (int i = 0; i < frame.getAttributes().size(); i++) {
					String tmp = frame.getAttributes().get(i).toString();
					int tmp2 = tmp.length() - 2;
					if (tmp.contains("svg:y")) y = tmp.substring(19, tmp2);
					else if (tmp.contains("width")) width = tmp.substring(23, tmp2);
					else if (tmp.contains("height")) height = tmp.substring(24, tmp2);
			...
			if (elem.getContentSize() != 0 && elem.getContent(0) instanceof Text) {
				...
				if (stName != null) {
					...
					if (superduper != null && superduper.startsWith("super ")) return new Element(
							"sup");
					if (superduper != null && superduper.startsWith("sub ")) return new Element(
							"sub");
				}
			}
	...
	private void init(org.jdom.Element elem) {
		...
		if (StyleManager.isBold(elem)) elements.add(new Element("b"));
		if (StyleManager.isItalic(elem)) elements.add(new Element("i"));
		if (StyleManager.isStriked(elem)) elements.add(new Element("strike"));
		if (StyleManager.isUnderlined(elem)) elements.add(new Element("u"));
	}
