...
public class MetricsSet<I> {
	...
	private final Collection<String> ignoredFlows;
	private final boolean ignoreCommentNodes;

	public MetricsSet(Collection<String> ignoredFlows, boolean ignoreCommentNodes) {
		this.ignoredFlows = ignoredFlows;
	...
	public boolean isIgnoreCommentNodes() {
		return ignoreCommentNodes;
	}
	...
	public boolean isIgnoredFlow(Flow flow) {

		for (String name : this.ignoredFlows) {
			if (flow.getName().equalsIgnoreCase(name)) return true;
		}
