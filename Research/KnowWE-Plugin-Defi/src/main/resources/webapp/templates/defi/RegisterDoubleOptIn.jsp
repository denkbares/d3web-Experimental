...
<script type='text/javascript'
		src='KnowWEExtension/scripts/KnowWE-Plugin-Defi-DoubleOptInRegistration.js'></script>
<form action="<wiki:CheckRequestContext
     context='login'><wiki:Link jsp='Login.jsp' format='url'><wiki:Param name='tab'
	  ...
	  id="editProfile"
	  class="wikiform"
	  onsubmit="registerUserDoubleOptIn();"
	...
	<c:if test="${param.tab eq 'profile'}">
		<div class="formhelp">
			<wiki:Messages div="error" topic="profile"
						   prefix='<%=LocaleSupport.getLocalizedMessage(pageContext,"prefs.errorprefix.profile")%>'/>
		</div>
	</c:if>
