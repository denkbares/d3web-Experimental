...
/**
 * DroolsRule contains a complete Drools rule
 * Rules start with `rule "name"' and end with `end'.
 * 
 * @author Alex Legler
 */
public class DroolsRule extends AbstractType {
	...
	static class DroolsRuleSubtreeHandler extends SubtreeHandler<DroolsRule> {
		...
		public void destroy(KnowWEArticle article, Section<DroolsRule> s) {
			Rule rule = (Rule) KnowWEUtils.getObjectFromLastVersion(article, s, rule_store_key);
			if (rule != null) {
