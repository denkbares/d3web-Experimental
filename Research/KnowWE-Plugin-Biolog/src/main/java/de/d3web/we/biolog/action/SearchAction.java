 ...
 * @author Jochen
 * 
 * This action is called when someone hits the search-button in the biolog-search mask
...
public class SearchAction extends DeprecatedAbstractKnowWEAction {
	...
	public String perform(KnowWEParameterMap parameterMap) {
		...
		if (searchText == null || searchText.length() == 0)
			return "<p class=\"box error\">Please enter a search text!</p>";
		...
		for (String searchProviderID : searchProviderKeys) {
			...
			resultBuffy.append("<div id=\"" + provider.getID()
					+ "\" class=\"panel\">");
			resultBuffy.append("<h3>" + verbalization + ": "+localResults.size()+" Treffer"+"</h3>");
			...
			//check whether searchprovider likes to render his results by himself 
			String rendered = provider.renderResults(resultList);
			if (rendered != null) {
