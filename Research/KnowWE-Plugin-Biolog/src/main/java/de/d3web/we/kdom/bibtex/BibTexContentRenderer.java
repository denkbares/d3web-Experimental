	...
	public void render(KnowWEArticle art, Section sec, KnowWEUserContext user, StringBuilder string) {
		...
		if (parseerror != null && parseerror.length() > 0) {
			...
			String dl="<p> (download all <a href=\"BibTexDownload.jsp?nodeID="
				+ sec.getID()+ "&"+KnowWEAttributes.TOPIC+"="+sec.getTitle()+"&filename=bibtex.bib\"><img width=16 height=16 title=\"Bibtex download\" src=\"KnowWEExtension/images/bibtex.png\" /></a>)" + "</p>";
			...
			for (Object cur : bibtexs.getEntries()) {
				...
				string.append(KnowWEUtils.maskHTML((String) BibTexRenderManager
					...
					string.append(KnowWEUtils.maskHTML("<p class=\"box error\"> Non parsable content: " +((BibtexToplevelComment)cur).getContent()+"</p>"));
				}
			}
			string.append(KnowWEUtils.maskHTML("</p>\n"));
