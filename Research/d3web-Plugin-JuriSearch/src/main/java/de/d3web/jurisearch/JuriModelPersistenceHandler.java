...
public class JuriModelPersistenceHandler implements KnowledgeReader,
        ...
        if (fatherquestion != null) {
            ...
            for (int i = 0; i < child_elements.getLength(); i++) {
                if (child_elements.item(i).getNodeName().equals(CHILD)) {
                    ...
                    List<Value> confirmingValues = new LinkedList<Value>();
    ...
    public Element createRuleElement(JuriRule jurirule, Document doc) {
        ...
        ruleelement.setAttribute(FATHER_QUESTION, jurirule.getParent().getName());

        for (Entry<QuestionOC, List<Value>> child : jurirule.getChildren().entrySet()) {
     ...
     * @param childelement
     */
    private Element createChildElement(Document doc, Entry<QuestionOC, List<Value>> child) {
        Element childelement = doc.createElement(CHILD);
        childelement.setAttribute(QUESTION, child.getKey().getName());
        for (Value value : child.getValue()) {
            if (!value.equals(Unknown.getInstance())
                    && UndefinedValue.isNotUndefinedValue(value)) {
                
                ChoiceValue choiceVal = (ChoiceValue)value;
                Element confirmingValue = doc.createElement(CONFIRMING_VALUE);
                confirmingValue.setTextContent(choiceVal.getAnswerChoiceID());
