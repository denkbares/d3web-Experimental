...
public class CoverageUtils {
	...
	private static Map<Flow, Collection<Flow>> createFlowStructure(KnowledgeBase base, Map<Flow, Collection<Flow>> result, Flow callingFlow) {
		...
		for (ComposedNode composedNode : composed) {
			Flow calledFlow = getCalledFlow(base, composedNode);
	 ...
	 * @return
	 */
	public static Flow getCalledFlow(KnowledgeBase base, ComposedNode composedNode) {
		String calledFlowName = composedNode.getCalledFlowName();
		Flow calledFlow = DiaFluxUtils.getFlowSet(base).get(calledFlowName);
		return calledFlow;
	}
