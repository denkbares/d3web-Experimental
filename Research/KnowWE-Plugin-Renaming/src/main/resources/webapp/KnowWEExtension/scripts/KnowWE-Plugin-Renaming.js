...
/**
 * The KNOWWE.renaming global namespace object. If KNOWWE.renaming is already defined,
 * the existing KNOWWE.core object will not be overwritten so that defined
 * namespaces are preserved.
 */
if (typeof KNOWWE.renaming == "undefined" || !KNOWWE.renaming) {
	KNOWWE.renaming = {};
}
...
KNOWWE.renaming = function(){
    ...
    return {
        ...
        init : function(){                    
            ...
            if( bttn ){
                _KE.add('click', bttn, KNOWWE.renaming.preview );
            }
        },
        /**
                    ...
                    fn : function(){
                        if(_KS('.check-select')) {
                             _KS('.check-select').each(function(element){
                                _KE.add('click', element, KNOWWE.renaming.selectPerSection );     
                             });
                        }
                            
                        if(_KS('.check-deselect')){
                            _KS('.check-deselect').each(function(element){
                                _KE.add('click', element, KNOWWE.renaming.deselectPerSection );     
                             });
                        }
                        if(_KS('#renaming-replace'))
                            _KE.add('click', _KS('#renaming-replace'), KNOWWE.renaming.replace );
                        
                        var imgs = _KS('.show-additional-text-renaming');
                        for( var i = imgs.length - 1; i > -1 ; i--){
                            _KE.add('click', imgs[i], KNOWWE.renaming.getAdditionalMatchText);
                        }
            ...
            var options = {
                ...
                response : {
                    ...
                    fn : function(){ 
                        ...
                        request = null;
                        _KE.add('click', _KS('#'+id), KNOWWE.renaming.getAdditionalMatchText);
                    }
                }
            }
