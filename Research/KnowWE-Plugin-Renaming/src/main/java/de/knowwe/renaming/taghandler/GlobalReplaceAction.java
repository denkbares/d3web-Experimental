	...
	private String perform(UserActionContext context) {
		...
		for (String string : replacementArray) {
			...
			if (findingsPerSection.containsKey(sec)) {
				findingsPerSection.get(sec).add(
						new WordBasedRenameFinding(start, length,
								WordBasedRenameFinding.getContext(
										start, sec, art.getRootSection().getText(), length),
								sec));
			}
			else {
		...
		for (Entry<Section<?>, List<WordBasedRenameFinding>> entry : findingsPerSection.entrySet()) {
			...
			for (WordBasedRenameFinding finding : list) {
				...
				lastEnd = start + finding.getLength();
				count++;
				// }
				// else {
				// String errorMsg =
				// rb.getString("KnowWE.renamingtool.msg.error").replace("{0}",
				// sec.getText());
