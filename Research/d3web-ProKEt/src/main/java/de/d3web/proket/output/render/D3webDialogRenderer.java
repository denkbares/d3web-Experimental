...
public class D3webDialogRenderer {
	 ...
	 */
	public String renderD3webQuestionnaire(TerminologyObject to, String parentID) {
		...
		for (TerminologyObject tochild : to.getChildren()) {
			if (tochild instanceof QContainer) {
				sb.append(renderD3webQuestionnaire(tochild, to.getId()));
			}
			else {

				sb.append(renderD3webQuestion(tochild, to.getId()));
			}
		}

		if (parentID != "") {
			makeTables(to.getId(), cc, sb);
		}
		...
		if (cols > 1) {
			String tableClosing = cc.tc.closeTable(to.getId());
			sb.append(tableClosing);
		}
	 ...
	 */
	public String renderD3webQuestion(TerminologyObject to, String parentID) {
		...
		for (TerminologyObject child : to.getChildren()) {
			// sb.append(renderD3webQuestion(child, to.getId()));
		}

		st.setAttribute("children", children);
		sb.append(st.toString());

		makeTables(parentID, cc, sb);
		return sb.toString();
	}
	...
	protected void makeTables(String parentID,
		...
		result.insert(0, cc.tc.getNextCellOpeningString(parentID, 1));

		// append table cell closing
		result.append(cc.tc.getNextCellClosingString(parentID));

		// add to the table container
		cc.tc.addNextCell(parentID);
	}
}
