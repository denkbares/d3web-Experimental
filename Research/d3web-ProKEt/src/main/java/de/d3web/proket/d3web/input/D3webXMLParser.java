...
public class D3webXMLParser {
    ...
    public GeneralDialogSettings.LoginMode getLoginMode() {
	if (XMLUtils.getStr((Element) dialogOpts, "login") != null) {
	    return GeneralDialogSettings.LoginMode.valueOf(
		    XMLUtils.getStr((Element) dialogOpts, "login").toUpperCase());
	}
	return GeneralDialogSettings.LoginMode.OFF;
    }
    ...
    // get the mode how to save cases (own filename vs. anonymized)
    public CaseSaveMode getCaseSaveMode() {
	if (XMLUtils.getStr((Element) dialogOpts, "saveMode") != null) {
	     return CaseSaveMode.valueOf(XMLUtils.getStr((Element) dialogOpts, "saveMode").toUpperCase());
	}
	return CaseSaveMode.OWN;
    }
