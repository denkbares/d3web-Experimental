 ...
 * 
 */
public class D3webDialogLogging extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (d3wcon.getKb() == null
            ...
            if (d3webParser.getLogging().equals("ON")) {
                d3wcon.activateLogging();
            }

            // TODO: refactor paths-to-set etc out to method that
            // can be overwritten by Servlet-specific methods.
    ...
    protected void addFacts(HttpServletRequest request,
        ...
        if (d3wcon.loggingActive()) {
            logger = d3wcon.getLogger();
            for (int i = 0; i < questions.size(); i++) {
                Date date = new Date();
                logger.logClickedObjects(questions.get(i), date.toString(), values.get(i));
                Session d3webSess = (Session) httpSession.getAttribute(D3WEB_SESSION);
                logger.writeJSONToFile(GlobalSettings.getInstance().getLogFolder()
                        + d3webSess.getId() + "_LOG.txt");
            }
        }
