...
public class QuestionManagerOld {
	...
	private Hashtable<String, QuestionOld> questions;
	
	
	public QuestionManagerOld() {
		questions = new Hashtable<String, QuestionOld>();
	}
	
	
	public void addQuestion(QuestionOld q) {
	...
	public QuestionOld getQuestion(String content) {
		return questions.get(getQuestionIndex(content));
	}
	
	
	public QuestionOld getRoot() {
		if (questions.size() >0) {
	...
	public String getXmlEncoding() {
		...
		LinkedList<QuestionOld> l = new LinkedList<QuestionOld>();
		l.addAll(questions.values());
		ListUtils.mergeSort(l, new BinaryRelation(){
			@Override
			public boolean inRelation(Object a, Object b) {
				QuestionOld qa = (QuestionOld)a;
				QuestionOld qb = (QuestionOld)b;
				return (qa.getId() < qb.getId());
			}
			
		});
		for (QuestionOld q : l) s = s + q.getXml() + "\n";
		s = s + "</dialog>";
