...
public class D3webNEW<KnoweldgeBase> extends HttpServlet {
	...
	private void show(HttpServletRequest request, HttpServletResponse response,
		...
		if (request.getParameter("reset") != null &&
				request.getParameter("reset").equals("true")) {
			Session newS = D3webUtils.createSession(d3wcon.getKb(), d3wcon.getDialogStrat());
			httpSession.setAttribute("d3webSession", newS);
			d3wcon.setSession(newS);
		}
		String dialog = loadDialog(httpSession);
	...
	private String loadDialog(HttpSession httpSession) {

		/*
		 * we need a pseudo-tree with a specific d3web root for chosing
		...
		((D3webDialog) root).setd3webRoot(d3wcon.getKb().getRootQASet());
		root.setId("d3webRoot");
		root.setCss(d3webParser.getCss());
		tree.setRoot(root);
		...
		D3webDialogRenderer rootRenderer = (D3webDialogRenderer) Renderer.getRenderer(tree.getRoot());
		// rootRenderer is a D3webDialogRenderer

		rootRenderer.renderD3web(cc, d3webSession);
