...
var questions;							// stores questions without zero choice questions
var markingQuestions;					// stores questions that need to be processed without the confirmation button
 ...
 * NOT in the d3web case currently as we here read the data from the knowledge
 * base and refresh the dialog accordingly
 */
function prepare_question_marking() {
	...
	markingQuestions.find("input,textarea").keyup(function() {
		remark_selectively($(this));
	});
	
	// and also input fields on confirm button click 
	markingQuestions.find("input,textarea").bind("click", function() {
		//remark_selectively($(this));
	});*/
}
 ...
 * Afterwards send the corresponding fact to the kb 
...
function flat_button_toggle(id) {
	...
	if (selected) {
		...
		// check if currently active and not selected
	...
	//if((target.attr("type") == null) || (target.attr("type") == "")){
		
	//} 
	
	// otherwise we need the value (e.g. input fields)
	//else {
		//answer = target.val();
	//}
		
	var qid = target.closest("div[id^='q_']").attr("id");
	qid = qid.substring(2); // remove "q_" to get the pure question id
	
	var answerid = target.attr("id").substring(2);
	
	// add the fact
	d3web_addfact(qid, answerid);
}
