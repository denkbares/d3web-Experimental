...
/**
 * 
 * One part of the case-markup-structure of the casetrain-wiki-format Represents
 * the end of a Case.
 * 
 * @author Jochen
 * @created 06.04.2011
	...
	public Closure() {
		...
		this.addSubtreeHandler(new GeneralSubtreeHandler<Closure>() {
			...
			public Collection<Message> create(KnowWEArticle article, Section<Closure> s) {
				...
				if (title == null) {
					messages.add(Utils.missingTitleError(Closure.class.getSimpleName()));
				}
				...
				if (plain == null) {
					messages.add(Utils.missingContentWarning(Closure.class.getSimpleName()));
				}
