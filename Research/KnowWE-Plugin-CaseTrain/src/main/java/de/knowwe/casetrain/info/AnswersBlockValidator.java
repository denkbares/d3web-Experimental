...
/**
 * 
 * Checks, if a specified Antwort is conform with its correlated Frage. Used in
 * SubtreeHandler of {@link Info} and in {@link AntwortKorrektheitContent}
 * 
 * TODO Only tests the line ({t} some text) if t has the right composition no
 * tests if the text is even there or so.
 * 
...
public class AnswersBlockValidator {
	...
	/**
	 * Return the Regex for finding supported FrageTypen as string.
	 * 
	 * @created 08.05.2011
	 * @return
	 */
	...
	/**
	 * Returns a list of FrageTypen, who have multiple AnswersBlocks
	 * 
	 * @created 10.05.2011
	 * @return
	 */
	...
	/**
	 * Validates the AnswersBlock according to its related Frage.
	 * 
	 * @created 28.04.2011
	 * @param frage
	 * @param answersBlock
	...
	private void checkMarkierung(
			Section<AnswersBlock> answersBlock, List<Message> messages, int minAnswers,
			boolean evaluation) {
		...
		if (found.size() < minAnswers) {
			messages.add(Utils.invalidArgumentError("Weniger als " + minAnswers
					+ " answersBlockBlock für Frage angegeben."));
		}
		...
		if (evaluation) {
			if (rightAnswers > 0) messages.add(Utils.invalidArgumentError(
					"Richtige Antwort für Frage nicht erlaubt"));
			if (numAnswers > 0) messages.add(Utils.invalidArgumentError("Mit Numerisch bewertete " +
					"Antwort für Frage nicht erlaubt"));
			return;
		}

		if (rightAnswers == 0) {
			messages.add(Utils.invalidArgumentError("Keine richtige Antwort für Frage angegeben"));
		}
		if ((wrongAnswers == 0) && (minAnswers > 1)) {
			messages.add(Utils.invalidArgumentWarning("Keine falsche Antwort für Frage angegeben"));
		}
		if ((numAnswers > 0) && ((rightAnswers > 0) || (wrongAnswers > 0))) {
			messages.add(Utils.invalidArgumentNotice("+/- ist gemischt mit Zahlen"));
		}
	...
	private void checkChoiceQuestion(Section<AnswersBlock> answersBlock,
			List<Message> messages, boolean evaluation) {
		...
		for (Section<AnswerLine> ans : found) {
			...
			if (antString.startsWith("{")) {
				...
				if (antString.indexOf("}") == -1) {
					messages.add(Utils.invalidArgumentError("Kein schließendes '}'"));
					continue;
				}

				antString = antString.substring(antString.indexOf("}") + 1);
				if (antString.length() == 0) messages.add(Utils.invalidArgumentError(
						"Leeres Wort eingegeben"));
			}
			...
			if (arg != null) {
				...
				if (c.equals("r")) {
					try {
						...
						messages.add(Utils.invalidArgumentError("Regex fehlerhaft: " + antString));
					}
					continue;
				}
				...
				try {
					...
					messages.add(Utils.invalidArgumentError("Kein gueltiges Zeichen innerhalb {}"));
					continue;
				}
	...
	private void checkNumanswersBlock(Section<AnswersBlock> answersBlock, List<Message> messages) {
		...
		for (Section<AnswerLine> ans : found) {
			...
			if ((interval.length == 0) || (interval.length > 2)) {
				messages.add(Utils.invalidArgumentError("Formatierung des Intervals fehlerhaft"));
				continue;
			}
			...
			try {
				...
				if (interval.length != 1) i2 = Integer.valueOf(interval[1]);
			}
			catch (Exception e) {
				messages.add(Utils.invalidArgumentError("Interval muss aus Zahlen bestehen"));
				continue;
			}

			if (i1 > i2) messages.add(Utils.invalidArgumentWarning(
	...
	private void checkPraefixPostfix(Section<AnswersBlock> answersBlock, List<Message> messages) {
		...
		if (found.size() == 0) messages.add(Utils.missingComponentWarning(
				Praefix.class.getSimpleName()));
