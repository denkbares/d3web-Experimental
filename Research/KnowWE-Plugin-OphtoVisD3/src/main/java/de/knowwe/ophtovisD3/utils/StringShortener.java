	...
	public String shorten(String startword){
		...
		case VOCALELIMIATION:
			...
			//	System.out.println(result + " " + currentPosition);
				...
				if(current=='a'||current=='A'||current=='e'||current=='E'||current=='i'||current =='I'||current=='o'||current=='O'||current=='u'||current=='U'){
					...
					result = result.substring(0, currentPosition)+result.substring(currentPosition+1, result.length());
				}
		...
		case MIDDLE:
			...
			if(dif%2==startword.length()%2){
				result=startword.substring(0, targetSize/2)+"..."+startword.substring(targetSize/2+dif,startword.length());
			}else{
			//	System.out.println("start = " + startword);
				result=startword.substring(0, targetSize/2-1)+"..."+startword.substring(targetSize/2+dif,startword.length());	
			}
			break;
