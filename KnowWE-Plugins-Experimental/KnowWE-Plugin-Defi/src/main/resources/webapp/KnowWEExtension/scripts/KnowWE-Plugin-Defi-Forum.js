...
function sendforumForm(id) {
	...
	if (document.getElementsByName(id + "_topic")[0].value.contains(";")) {
		defiAlert("Ungültige Überschrift (Verwenden Sie bitte kein Semikolon).");
		return;
	}
...
function newChat(user1, user2) {
	...
	var options = {
		...
		response : {
			...
			fn : function() {
				setTimeout(
						'window.location = "Wiki.jsp?page=Persoenliche Nachrichten('
								+ user1 + ',' + user2 + ')"', 200);
			}
		}
	}
...
function loadWindow() {
	...
	alertDiv = document.getElementsByTagName("body")[0].appendChild(document
			.createElement("div"));
	alertDiv.id = "alertDialog";
	alertDivModal = document.getElementsByTagName("body")[0]
			.appendChild(document.createElement("div"));
	alertDivModal.id = "alertDialogModal";

	// position - alert-box
	alertDiv.style.top = (window.innerHeight - 100 - alertDiv.offsetHeight) / 2
			+ "px";
	alertDiv.style.left = (document.documentElement.scrollWidth - alertDiv.offsetWidth)
			/ 2 + "px";
	// position - background
